...
# Block fruit prefixes of length 3 or more:
# app -> apple, gra -> grape, man -> mango, etc.
for fruit in BLOCKED_OUTPUT_PATTERNS:
    for i in range(3, len(fruit) + 1):
        BLOCKED_PREFIX_PATTERNS.append(fruit[:i])

# Block short high-risk fruit prefixes.
BLOCKED_PREFIX_PATTERNS.extend([
    "fi",  # fig
    "li",  # lime
    "ki",  # kiwi
    "pl",  # plum
...
def clean_letters(text: str) -> str:
    return "".join(ch for ch in text.lower() if ch in STOI)


def contains_blocked_fruit(text: str) -> bool:
    clean = clean_letters(text)
    return any(fruit in clean for fruit in BLOCKED_OUTPUT_PATTERNS)
...
def is_safe_request(prefixes: List[str]) -> Optional[str]:
    ...
    for prefix in prefixes:
        clean = clean_letters(prefix)

        if not clean:
            continue

        if contains_blocked_fruit(clean):
            return "request contains a banned fruit name."

        for bad_prefix in BLOCKED_PREFIX_PATTERNS:
            if clean.endswith(bad_prefix):
                return "request prefix could lead to a banned fruit name."
...
def is_safe_name(name: str) -> bool:
    ...
    """
    return not contains_blocked_fruit(name)
